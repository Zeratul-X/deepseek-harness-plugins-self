...
| 📎 @ 文件引用 | `plugins/file-ref` | 输入框输入 `@` 弹出文件筛选(只列当前展开工作区的文件),选中后在输入框上方 dock 显示完整路径引用(输入框内不显示 chip,保持纯净);点击 dock 弹代码预览选行,可插入纯文本 `@path line a-b`,`✕` 移除引用;预览内点击行号选行、Shift 点击/拖拽连选、Ctrl+F 搜索定位 |
