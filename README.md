...
| 📎 @ 文件引用 | `plugins/file-ref` | 输入框输入 `@` 弹出文件筛选(只列当前展开工作区的文件),选中插入 `@相对路径` 引用;引用以完整路径显示在输入框上方 dock(点击弹代码预览选行,可插入 `@path line a-b`,`✕` 移除引用),预览内点击行号选行、Shift 点击/拖拽连选、Ctrl+F 搜索定位;输入框内 chip 同步显示路径文本并适配暗色主题 |
